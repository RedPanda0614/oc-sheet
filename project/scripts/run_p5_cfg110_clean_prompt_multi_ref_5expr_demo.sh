#!/bin/bash
#SBATCH --job-name=oc-p5-clean-demo
#SBATCH --account=cis260099p
#SBATCH --partition=GPU-small
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --time=01:00:00
#SBATCH --output=logs/%j_p5_clean_cfg110_multi_ref_5expr_demo.log
#SBATCH --error=logs/%j_p5_clean_cfg110_multi_ref_5expr_demo.err
...
RESULT_DIR="results/p5_cfg110_clean_prompt_multi_ref_5expr_demo"
SOURCE_PAIRS_JSON="results/p5_cfg110_multi_ref_5expr_demo/pairs.json"
...
cd "${PROJECT_DIR}"
cp "${SOURCE_PAIRS_JSON}" "${PAIRS_JSON}"
...
python inference/batch_inference_p5_rerank_labeled.py \
  ...
  --num-candidates 4 \
  --prompt-style clean \
  --model-tag "p5_cfg110_clean_prompt_multi_ref_5expr_demo"
...
root = Path("results/p5_cfg110_clean_prompt_multi_ref_5expr_demo")
...
echo
echo "Saved P5 clean-prompt CFG110 demo to: ${PROJECT_DIR}/${RESULT_DIR}"
