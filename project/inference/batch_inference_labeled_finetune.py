...
from run_baseline import EMOTION_PROMPTS, NEGATIVE_PROMPT, load_all_models, load_all_models_finetuned


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--pairs-json", default="data/label_pairs/val.json", help="带 target_emotion 标签的验证集 JSON")
    ...
    parser.add_argument("--ip-weight-name", default="ip-adapter-plus_sd15.bin", help="IP-Adapter 权重文件名")
    parser.add_argument("--n", type=int, default=500, help="测试数量，设为 0 表示跑完整个 JSON")
    parser.add_argument("--ft-checkpoint", default="results/ip_adapter_finetune/checkpoints/step_002000", help="微调 checkpoint 目录（含 image_proj_model.pt / ip_attn_procs.pt），不传则使用基础 IP-Adapter")
...
def main():
    ...
    if args.ft_checkpoint:
        pipe, _, device = load_all_models_finetuned(args)
    else:
